...
public partial class FptBookingContext : DbContext
{
    ...
    /*
     * Q21: T?i sao dùng DateOnly cho BookingDate?
     * A21: 
     * - DateOnly: Ch? luu ngày, không có time (phù h?p v?i booking date)
     * - DateTime: Có c? ngày và gi? (không c?n thi?t cho booking date)
     * - Uu di?m: Rõ ràng hon, không lo l?ng v? timezone, ti?t ki?m storage
     * - Luu ý: DateOnly ch? có t? .NET 6+, c?n SQL Server 2008+
     */
    protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
    {
         ...
         * A22: 
         * - Hi?n t?i: Hardcoded trong code (KHÔNG AN TOÀN cho production)
         * - Nên: Connection string du?c config trong appsettings.json (dã làm)
         * - Production: Dùng Azure Key Vault, AWS Secrets Manager, ho?c Environment Variables
         * - Luu ý: Không commit connection string vào git
         */
        // Connection string du?c config trong Program.cs qua DI
        // Không c?n config ? dây n?a
    }

    /*
     * Q23: Có index cho các c?t thu?ng query không?
     * A23: CHUA có index tùy ch?nh.
     * - EF Core t? t?o index cho Primary Key và Foreign Key
     * - Nên thêm index cho:
     *   + Users.Email (dã có unique index)
     *   + Bookings.BookingDate, Status, FacilityId (query thu?ng xuyên)
     *   + Bookings.UserId (query booking c?a user)
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        modelBuilder.Entity<Booking>(entity =>
        {
            ...
            // TODO: Thêm index d? t?i uu query
